...
// export default function AddGoodsModal({ onClose, onAddProduct, productToEdit, onUpdateProduct }) {
...
export default function AddGoodsModal({ onClose, onAddProduct, productToEdit, onUpdateProduct, }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (productToEdit) {
        // 编辑模式
        const response = await axios.put(`http://127.0.0.1/update/${product.id}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        alert("修改成功！");
        if (onUpdateProduct) onUpdateProduct(response.data.data);
    ...
    <div className="modal-overlay">
      <div className="modal">
        ...
        <form onSubmit={handleSubmit} className="form">
          ...
          <div className="actions">
            <button type="submit">{productToEdit ? "保存修改" : "提交"}</button>
            <button type="button" onClick={onClose}>
              取消
            </button>
          </div>
